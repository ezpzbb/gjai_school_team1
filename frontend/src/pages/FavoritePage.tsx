...
const FavoritePageContent: React.FC = () => {
  ...
  const handleOpenRoiEditor = (cctvId: number, streamUrl: string | null) => {
    if (!streamUrl) return;
    setRoiCctvId(cctvId);
    setRoiStreamUrl(streamUrl);
    setRoiModalOpen(true);
  };

  const handleCloseRoiEditor = () => {
    setRoiModalOpen(false);
    setRoiCctvId(null);
    setRoiStreamUrl(null);
  };

  const { selectedCCTVs, pendingCCTV, placeCCTVAt, setSelectedCCTVs, analysisMode, setAnalysisMode, analysisTargetId, setAnalysisTargetId, focusAndExpandCCTV } =
    favoritePageContext;
  const { cctvLocations, favorites, addFavorite, removeFavorite, refreshFavorites, error: dataError } = useData();
  ...
  const renderCameraCard = (cctv: CCTV | undefined, index: number, isCompact: boolean) => {
    ...
    const cardStyle: React.CSSProperties = isCompact
      ...
      : {
          ...
          transition: isExpanded || isAnimating ? "all 0.4s cubic-bezier(0.4, 0, 0.2, 1)" : "all 0.3s ease",
          ...(canPlace && !isExpanded
            ? {
    ...
    return (
      <div key={cctv ? `cctv-${cctv.cctv_id}` : `empty-${index}`} onClick={handleCardClick} className={baseClassName} style={cardStyle}>
        {cctv ? (
          <>
                ...
                <Camera
                  ...
                  onToggleFavorite={() =>
                    handleToggleFavorite(
                      cctv.cctv_id,
                      favorites.some((fav) => fav.cctv_id === cctv.cctv_id)
                    )
                  }
                  onExpand={
                    isCompact
                  ...
                  pageType="favorite"
                  onOpenRoiEditor={handleOpenRoiEditor}
                />
                {/* 화면 중앙에 크게 뜨는 모달 – Camera 바깥에서 렌더 */}
                {roiModalOpen && roiCctvId !== null && roiStreamUrl && <RoiEditorModal cctvId={roiCctvId} streamUrl={roiStreamUrl} onClose={handleCloseRoiEditor} />}
              </div>
            </div>
          </>
        ) : (
          <div
            className={`w-full h-full flex items-center justify-center transition-all ${canPlace && !isCompact ? "" : "bg-gray-50 dark:bg-gray-700/50"}`}
            style={
              canPlace && !isCompact
                ? {
                    background: "rgba(53, 122, 189, 0.15)",
          ...
          >
            <div className="text-center">
              ...
              >
                {canPlace && !isCompact ? `클릭하여 "${pendingCCTV.location}" 배치` : "지도에서 CCTV 선택"}
              </div>
              {canPlace && !isCompact && (
                <div
  ...
  if (error) {
    return (
      <div className="text-red-600 dark:text-red-400 text-center p-4">
        {error}
        <button className="mt-4 bg-blue-600 dark:bg-blue-700 text-white px-4 py-2 rounded hover:bg-blue-700 dark:hover:bg-blue-800 transition" onClick={handleRefreshFavorites}>
          재시도
        </button>
      </div>
    );
