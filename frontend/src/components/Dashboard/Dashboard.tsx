...
const Dashboard: React.FC = () => {
  ...
  const hasMapProvider = typeof mapContext.registerSelectCCTV === "function" && mapContext.registerSelectCCTV.toString().indexOf("console.warn") === -1;
  ...
  if (error) {
    return (
      ...
      >
        ...
        >
          <svg className={`w-4 h-4 transition-transform duration-300 ${dashboardCollapsed ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
        </button>
        {!dashboardCollapsed && <div className="w-full h-full flex items-center justify-center text-center text-sm text-red-500 dark:text-red-300">{error}</div>}
      </div>
    );
  }
        ...
        <>
          ...
          {favoriteCCTVs.length > 0 && (
            <div className="mb-6 flex-shrink-0">
              ...
              <div className="flex flex-col gap-2 overflow-y-auto" style={{ maxHeight: "calc((100vh - 2rem - 4rem - 0.5rem - 2rem) * 0.4)" }}>
                {favoriteCCTVs.map(({ favorite, cctv }) => (
                  <div
                    ...
                    className={`text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-700 rounded-lg p-3 transition flex-shrink-0 ${
                      hasMapProvider || favoritePageContext ? "hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer" : "cursor-default"
                    }`}
                  >
          ...
          {recentEvents.length > 0 && (
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="flex flex-col gap-3 overflow-y-auto flex-1" style={{ maxHeight: "calc((100vh - 2rem - 4rem - 0.5rem - 2rem) * 0.5)" }}>
                {recentEvents.map((event) => (
                  <div
                    ...
                    className={`bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3 text-gray-900 dark:text-gray-100 transition flex-shrink-0 ${
                      hasMapProvider ? "hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer" : "cursor-default"
                    }`}
                  >
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">{formatEventDate(event.startDate)}</div>
                  </div>
                ))}
              </div>
            </div>
