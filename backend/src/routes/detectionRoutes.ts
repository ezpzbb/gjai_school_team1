  ...
  router.post("/detection/image", upload.single("image"), async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        // 프레임 정보 조회
        const [frameRows] = await conn.query<any[]>(`SELECT cctv_id, timestamp FROM frame WHERE frame_id = ?`, [frameId]);
        ...
        // DB에 프레임 이미지 경로 업데이트
        await conn.query(`UPDATE frame SET image_path = ? WHERE frame_id = ?`, [relativePath, frameId]);
        ...
        if (frameCount > MAX_FRAMES_PER_CCTV) {
          ...
          for (const oldFrame of oldestFrames) {
            ...
            await conn.query(`UPDATE frame SET image_path = ? WHERE frame_id = ?`, [DELETED_FLAG, oldFrame.frame_id]);
  ...
  router.post("/detection", async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        if (!actualFrameId) {
          const [frameRows] = await conn.query<any[]>(`SELECT frame_id FROM frame WHERE cctv_id = ? ORDER BY timestamp DESC LIMIT 1`, [cctvId]);
          if (frameRows.length > 0) {
        ...
        if (congestionId !== null && congestionLevel >= threshold) {
          setImmediate(async () => {
            try {
              await congestionNotificationService.sendImmediateNotification(congestionId!, cctvId, congestionLevel);
              console.log(`[Detection] 혼잡도 알림 발송: CCTV ${cctvId}, 혼잡도 ${congestionLevel} (임계값: ${threshold}), Congestion ID ${congestionId}`);
            } catch (error: any) {
