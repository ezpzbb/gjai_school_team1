...
async function saveCctvToDatabase(
  ...
  try {
    ...
    const cctvCode = crypto.createHash("sha256").update(`${location.trim()}|${longitude}|${latitude}`).digest("hex");
    ...
    throw new Error(
      `DB 저장 실패 (cctv_id: ${cctvId}, location: ${location}): ${errorMessage}${sqlMessage ? ` (SQL: ${sqlMessage})` : ""}${sqlState ? ` [${sqlState}]` : ""}${
        errno ? ` (errno: ${errno})` : ""
      }`
    );
  }
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
            ...
            if (streamUrlFromXlsx && streamUrlFromXlsx.startsWith("http")) {
              ...
              if (centerName === "광주광역시" && (!cctvch || !id)) {
                if (gwangjuCctvMapping[cctvId]) {
                  console.log(
                    `[${i + 1}/${rows.length}] ✅ 매핑 테이블에서 URL 생성: ${cctvName} (cctvch=${gwangjuCctvMapping[cctvId].cctvch}, id=${gwangjuCctvMapping[cctvId].id})`
                  );
                } else {
          ...
          // DB 저장 (연결 객체 전달)
          const result = await saveCctvToDatabase(connection, cctvId, cctvName, latitude, longitude, streamUrl, centerName);
