...
def _get_stream_url_from_backend(cctv_id: int) -> str:
    """
    backend의 CCTV API를 통해 DB에 저장된 api_endpoint를 조회하고,
    모델 입력용 스트림 URL로 사용한다.
    """
    try:
        resp = requests.get(f"{BACKEND_BASE}/api/cctv/locations", timeout=2.0)
        resp.raise_for_status()
    except Exception as e:
        raise HTTPException(
            status_code=502, detail=f"backend CCTV API 호출 실패: {e}") from e

    payload = resp.json()
    items = payload.get("data", payload)

    for item in items:
        if int(item.get("cctv_id")) == cctv_id:
            url = item.get("api_endpoint")
            if not url:
                raise HTTPException(
                    status_code=502, detail="해당 CCTV의 api_endpoint가 없습니다.")
            return url

    raise HTTPException(
        status_code=404, detail=f"cctv_id={cctv_id} CCTV를 찾을 수 없습니다.")
